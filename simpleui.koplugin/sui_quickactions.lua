...
local function _registerBuiltins()
    ...
    local builtins = {
        ...
        },
        {
            id    = "browse_added",
            label = _("Date Added"),
            icon  = Config.ICON.added,
            browsemeta_mode = "added",
            is_in_place = false,
            execute = function(ctx)
                local su = ctx.show_unavailable or _unavailToast
                local ok_bm, BM = pcall(_BM)
                ...
                local fc = fm and fm.file_chooser
                if not fc then return end
                if ctx.already_active then BM.navigateToRoot(fc, fm, "added")
                else BM.navigateTo(fm, "added") end
            end,
        },
        {
            id    = "browse_publication",
            label = _("Publication Date"),
            icon  = Config.ICON.publication,
            browsemeta_mode = "publication",
            is_in_place = false,
            execute = function(ctx)
                local su = ctx.show_unavailable or _unavailToast
                local ok_bm, BM = pcall(_BM)
                ...
                local fc = fm and fm.file_chooser
                if not fc then return end
                if ctx.already_active then BM.navigateToRoot(fc, fm, "publication")
                else BM.navigateTo(fm, "publication") end
            end,
        },
    }
