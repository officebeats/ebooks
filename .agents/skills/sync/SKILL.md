...
## Step 2: Optimize and Clean Up Kindle Storage
Execute the `optimize_kindle.py` script for each Kindle to ensure performance is optimized, unnecessary plugins are removed, and storage is cleaned of bloat (old logs, orphaned SDR folders).
...
Read `kindle_hosts.json` to get the list of active Kindles (by their nickname keys, e.g. `older`, `newer`, `white`).
