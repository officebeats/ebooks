...
1. **Launcher-Only Native Amazon Home Screen**: Isolates `/mnt/us/documents/` so ONLY KOReader and KUAL launchers appear on the native Amazon Kindle Home Screen. All ebooks are moved to `/mnt/us/epubs/`, and all native Amazon user guides, sample booklets, and non-launcher logs are purged from the home screen directory.
